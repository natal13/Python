...
def get_triangles_vertices(triangles, vertices):
    ...
    for triangle in triangles:
        new_vertices = [vertices[triangle[0]], vertices[triangle[1]], vertices[triangle[2]]]
        triangles_vertices.append(new_vertices)
...
moveVector = np.ndarray(3, buffer = np.array([-xm+9000, -ym+9000, 0]))
...
cl, ind = cloud_selected.remove_statistical_outlier(nb_neighbors=3, std_ratio=2)
...
o3d.io.write_point_cloud("clouds.pts", downpcd, write_ascii=True, compressed=False)
...
volume = reduce(lambda a, b:  a+volume_under_triangle(b), get_triangles_vertices(np.array(surface.triangles),np.array( surface.vertices)), 0)
